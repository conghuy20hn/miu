...
const actionToggleWatchLater = async function(req, res){
    const body = req.body;
    let valid = vnVideoService.validToggleWatchlate(body);
    if(valid!==true){        
        res.json(valid);
    }else{
        ...
        if(video === false || !video.id){
            res.json({responseCode: ResponseCode.NOT_FOUND, message : 'Video không tồn tại.'});
        }else{
            ...
            if(videoStr === false || !videoStr) videoStr="";
            let arrVideo = videoStr.split(',');
            let video_list_watch_later 
            ...
            if (status == 0) {
                ...
                if (videoStr) {
                    ...
                    redisService.setKey(keyRedis, videoStr, false, dbRedis);
                } else {
                    redisService.setKey(keyRedis, id, false, dbRedis);
                    // $redisCache->set("video_list_watch_later_" . $this->userId, $id);
                }
            }
            res.json({responseCode: ResponseCode.SUCCESS, message : ResponseCode.getMessage(ResponseCode.SUCCESS), data:{isWatchLater:isWatchLater}});
        }
    }
}
...
actionGetFriendsVideo = async function(req, res){
    const body = req.body;
    // let valid = vnVideoService.validGetFriendsVideo(body);
        ...
        switch (filterType.toUpperCase()) {
            case "VIDEO":
                boxUserFollow = await UserFollowObj.serialize(
                    ...
                    {userId: userId, limit: 4, offset: 0}
                );
                break;
            case "MUSIC":
                boxUserFollow = await UserFollowObj.serialize(
