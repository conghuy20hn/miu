...
class GalleryBehavior extends Behavior
{
    ...
    public function getImages()
    {
        if ($this->_images === null) {
            ...
            $imagesData = $query
                ->select(['id', 'name', 'description', 'rank', 'file_path'])
                ->from($this->tableName)
    ...
    public function addFileArticle($imageFile, $dir, $url)
    {
        $extension = $imageFile->extension;
        $fileName = str_replace('.' . $extension, '', $imageFile->name);
        $db = \Yii::$app->db;
        $db->createCommand()
            ->insert(
                $this->tableName,
        ...
        $id = $db->getLastInsertID('gallery_image_id_seq');
        $dirUploads = $dir . '/' . $this->getGalleryId() . '/' . $id . '/' . md5($imageFile->name) . '.' . $extension;
        $filePath = $url . $this->getGalleryId() . '/' . $id . '/' . md5($imageFile->name) . '.' . $extension;
//        $filePath = \Yii::$app->params['article_file_upload_path'] . $this->getGalleryId() . '/' . $id . '/' . md5($imageFile->name) . '.' . $extension;
        $dirUrl = str_replace($dir . '/' . $this->getGalleryId(), $url . '/' . $this->getGalleryId(), $dirUploads);
        $this->createFolders($this->getFilePath($id, 'original'));
        move_uploaded_file($imageFile->tempName, $dirUploads);
        $db->createCommand()
            ->update(
                $this->tableName,
                ['rank' => $id, 'file_path' => $filePath],
                ['id' => $id]
            )->execute();
    ...
    public function deleteFilesArticle($imageIds)
    {
        foreach ($imageIds as $imageId) {
            $this->deleteImage($imageId);
        }
    ...
    public function deleteFileArticle($imageId, $path)
    {
        $db = \Yii::$app->db;
        $db->createCommand()
            ->delete(
        ...
        foreach ($imageId as $itemsId) {
            $this->delTree($path . '/' . $itemsId);
        }
