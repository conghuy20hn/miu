...
class GalleryManagerAction extends Action
{
    ...
    {
        if (in_array($this->type, Yii::$app->params['type_galery_upload'])) {
            $this->behavior->deleteFileArticle($ids, $this->behavior->directory . '/' . $this->galleryId);
        } else {
    ...
    public function actionAjaxUpload()
    {
        if (in_array($this->type, Yii::$app->params['type_galery_upload'])) {
            ...
            /*if (move_uploaded_file($imageFile->tempName, Yii::getAlias('@webroot') . '/upload/images/article/gallery/' . $imageFile->name)) {
                $success = true;
                $paths[] = "/upload/media1/file_article/";
            } else {
                $success = false;
            }*/
            $extensionConfig = explode(',', $this->behavior->extension);
            $extension = $imageFile->extension;
            if (!in_array($extension, $extensionConfig)) {
                Yii::$app->response->headers->set('Content-Type', 'text/html');
                throw new ForbiddenHttpException;
            }
            $image = $this->behavior->addFileArticle($imageFile, $this->behavior->directory, $this->behavior->url);



//            move_uploaded_file($imageFile->tempName, Yii::getAlias('@webroot') . '/upload/images/article/gallery/' . $this->galleryId . '/' . $imageId . '/' . md5($imageFile->name) . '.' . $extension);
            Yii::$app->response->headers->set('Content-Type', 'text/html');
            return Json::encode(
                array(
                    'id' => $image['id'],
